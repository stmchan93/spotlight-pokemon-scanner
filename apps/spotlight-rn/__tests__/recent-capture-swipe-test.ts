...
describe('recent capture swipe gestures', () => {
  ...
  it('clamps the reveal distance in both closed and open states', () => {
    ...
    expect(clampRecentCaptureSwipeTranslate(-200)).toBe(-(recentCaptureDeleteRevealWidth * 2));
    expect(clampRecentCaptureSwipeTranslate(40, true)).toBe(-recentCaptureDeleteRevealWidth * 2 + 40);
    expect(clampRecentCaptureSwipeTranslate(200, true)).toBe(0);
    expect(clampRecentCaptureSwipeTranslate(-16, true)).toBe(-(recentCaptureDeleteRevealWidth * 2));
  });
});
