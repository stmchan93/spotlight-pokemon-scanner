...
export function AccountScreen() {
  ...
  */
  ...
  /*
    Retire this device's push token BEFORE the session goes away, so the next
    account on this phone cannot inherit the previous owner's deal alerts. It
    has to happen here rather than after `signOut()` because the revoke is an
    authenticated write. Awaited, but never allowed to block the sign-out.
  */
  const handleSignOut = useCallback(async () => {
    await revokePushToken(spotlightRepository).catch(() => false);
    await auth.signOut();
