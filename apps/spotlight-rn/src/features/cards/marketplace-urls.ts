...
/**
 * "See all on eBay" query derived from REAL sold-listing titles (Scrydex's
 * recent-sales lane). Our synthesized name/set/number query only matches
 * listings worded the way we guessed; sellers' actual phrasing is what eBay
 * indexes, so a query built from tokens the real sold titles SHARE captures
 * far more of the true solds. eBay has no multi-item URL, so a majority-token
 * sold search is the closest possible "all of these" aggregate.
 *
 * Falls back to buildEbaySearchUrl when there aren't enough titles to mine.
 */
export function buildEbaySoldsUrlFromSales(
  titles: readonly string[],
  fallback: Parameters<typeof buildEbaySearchUrl>[0],
): string | null {
  ...
  if (cleanedTitles.length >= 2) {
    // Token frequency across titles (counted once per title). Keep the first
    // encountered casing for readability; eBay matching is case-insensitive.
    const counts = new Map<string, { count: number; display: string; firstIndex: number }>();
      ...
      title
        ...
        .forEach((token, index) => {
          ...
          if (existing) {
            ...
            counts.set(key, { count: 1, display: token, firstIndex: index });
          }
        });
    });

    const majority = Math.ceil(cleanedTitles.length / 2);
    const graderToken = cleanedMarketplaceToken(fallback.grader)?.toLowerCase() ?? null;
    const gradeToken = cleanedMarketplaceToken(fallback.grade)?.toLowerCase() ?? null;
    const sharedTokens = [...counts.entries()]
      .filter(([key, entry]) =>
        entry.count >= majority
        // grader+grade join as ONE quoted phrase below — drop their loose tokens.
        && key !== graderToken
        && key !== gradeToken)
    ...
    if (sharedTokens.length >= 3) {
      const gradePhrase =
        graderToken && gradeToken ? `"${fallback.grader} ${fallback.grade}"`.replace(/\s+/g, ' ') : null;
      const query = [gradePhrase, ...sharedTokens].filter(Boolean).join(' ');
      return buildEbaySoldSearchUrl(query);
    }
  }

  return buildEbaySearchUrl(fallback);
}
