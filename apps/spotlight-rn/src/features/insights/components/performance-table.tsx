...
// Frozen-first-column table (Figma 2100-1755): the card identity column stays
...
const METRIC_COLUMNS = ['Chart', 'Current', '$G/L', '%G/L', '$Total', 'Cost'] as const;
            ...
            {METRIC_COLUMNS.map((label) => (
              <Text
                ...
                style={[
                  ...
                  { color: theme.colors.gray600, width: label === 'Chart' ? CHART_W : CELL_W },
                ]}
              >
                {label}
              </Text>
          ...
          <View style={styles.rowStack}>
            {rows.map((row) => (
              <View key={row.entryId} style={styles.metricRow}>
                ...
                <Text style={[theme.typography.body, styles.cell, { color: gainColor(row.ytdGainDollar) }]}>
                  {money(row.ytdGainDollar)}
                </Text>
                <Text style={[theme.typography.body, styles.cell, { color: gainColor(row.ytdGainPercent) }]}>
                  {row.ytdGainPercent == null ? '—' : `${Math.round(row.ytdGainPercent)}%`}
                </Text>
                <Text style={[theme.typography.body, styles.cell, { color: theme.colors.gray900 }]}>
