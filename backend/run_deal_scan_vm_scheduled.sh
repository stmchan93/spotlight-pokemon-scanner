...
if [ -z "$FLOCK_BIN" ]; then
  echo "flock is required to run scheduled deal scans." >&2
  exit 1
fi

if ! "$PYTHON_BIN" "$SCRIPT_DIR/vm_sync_schedule.py" \
  --cron "$DEAL_SCAN_CRON_SCHEDULE" \
  --timezone "$SYNC_CRON_TIMEZONE" \
  --should-run-now; then
  exit 0
fi

if ! "$FLOCK_BIN" -n "$DEAL_SCAN_LOCK_FILE" "$SCRIPT_DIR/run_deal_scan_vm.sh" >> "$DEAL_SCAN_LOG_FILE" 2>&1; then
  printf '[deal-scan-scheduler] skipped because lock is already held for %s (%s)\n' \
    "$DEAL_SCAN_CRON_SCHEDULE" "$SYNC_CRON_TIMEZONE" >> "$DEAL_SCAN_LOG_FILE"
fi
