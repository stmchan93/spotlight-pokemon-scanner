...
TCGCSV_SYNC_CRON_SCHEDULE="${SPOTLIGHT_VM_TCGCSV_SYNC_CRON:-5 13 * * *}"
SYNC_CRON_TIMEZONE="${SPOTLIGHT_VM_SYNC_CRON_TZ:-America/Los_Angeles}"
TCGCSV_SYNC_LOCK_FILE="${SPOTLIGHT_TCGCSV_SYNC_LOCK_FILE:-$SCRIPT_DIR/data/tcgcsv-sync.lock}"
TCGCSV_SYNC_LOG_FILE="${SPOTLIGHT_TCGCSV_SYNC_LOG_FILE:-$SCRIPT_DIR/logs/tcgcsv_sync.log}"
...
if ! "$PYTHON_BIN" "$SCRIPT_DIR/vm_sync_schedule.py" \
  --cron "$TCGCSV_SYNC_CRON_SCHEDULE" \
  --timezone "$SYNC_CRON_TIMEZONE" \
  --should-run-now; then
  exit 0
fi

if ! "$FLOCK_BIN" -n "$TCGCSV_SYNC_LOCK_FILE" "$SCRIPT_DIR/run_tcgcsv_sync_vm.sh" >> "$TCGCSV_SYNC_LOG_FILE" 2>&1; then
  printf '[tcgcsv-sync-scheduler] skipped because lock is already held for %s (%s)\n' \
    "$TCGCSV_SYNC_CRON_SCHEDULE" "$SYNC_CRON_TIMEZONE" >> "$TCGCSV_SYNC_LOG_FILE"
fi
