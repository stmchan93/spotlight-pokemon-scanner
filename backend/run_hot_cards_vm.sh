...
# Hot on Ekalight table (distinct-user attention counts). Hourly via the
# deploy_to_vm.sh crontab. Writes a small precomputed table in this box's own
# database; the feed endpoint only reads it. Same flag as the endpoint, so the
# cron line is a no-op until the env turns the feature on.
case "$(printf '%s' "${HOT_CARDS_ENABLED:-0}" | tr '[:upper:]' '[:lower:]')" in
  1|true|yes|on) ;;
  *)
    echo "[hot-cards] $(date -u +"%Y-%m-%dT%H:%M:%SZ") HOT_CARDS_ENABLED is off; skipping"
    exit 0
    ;;
esac
...
export SPOTLIGHT_RUNTIME_LABEL="${SPOTLIGHT_RUNTIME_LABEL:-vm-hot-cards:${HOSTNAME_VALUE}}"

echo "[hot-cards] $(date -u +"%Y-%m-%dT%H:%M:%SZ") computing"
# "$@" forwards manual flags; the cron passes none.
cd "$SCRIPT_DIR"
"$PYTHON_BIN" "$SCRIPT_DIR/hot_cards.py" --database-path "$DATABASE_PATH" "$@"
