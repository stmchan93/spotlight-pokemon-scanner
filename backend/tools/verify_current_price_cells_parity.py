...
def _day_cells(connection: sqlite3.Connection, card_id: str, price_date: str) -> list[sqlite3.Row]:
    ...
    return connection.execute(
        "SELECT * FROM card_price_history_cell WHERE card_id = ? AND price_date = ? AND provider = ?",
...
def main() -> int:
    ...
    def _compare(card_id: str, pricing_context, tag: str) -> None:
        ...
        if _sig(json_pricing, PRICE_FIELDS) != _sig(cells_pricing, PRICE_FIELDS):
            price_mismatches.append(
                (card_id, tag, _sig(json_pricing, PRICE_FIELDS), _sig(cells_pricing, PRICE_FIELDS))
            )
    ...
    print(f"PRICE-FIELD mismatches:        {len(price_mismatches)}")
    if args.include_payload:
        print(f"payload/trendsPct mismatches:  {len(payload_mismatches)} (expected, informational)")
    print("=" * 70)

    if price_mismatches:
        print(f"\nFirst {min(args.show, len(price_mismatches))} PRICE mismatches:")
        for card_id, tag, json_sig, cells_sig in price_mismatches[: args.show]:
        ...
        print("\nRESULT: MISMATCH — cells path diverges on price fields. Investigate above.")
        return 1
