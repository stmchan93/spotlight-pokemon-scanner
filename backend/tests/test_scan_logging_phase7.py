...
class ScanLoggingPhase7Tests(unittest.TestCase):
    ...
    def test_create_deck_entry_persists_user_overrode_mismatch_warning(self) -> None:
        # Each warning kind round-trips into scan_events; default (omitted) stays NULL.
        for kind_index, kind in enumerate(("language", "condition", "both")):
            scan_id = f"scan-mismatch-override-{kind}"
            card_id = f"mismatch-card-{kind}"
            self._seed_scan_event_for_artifacts_json(scan_id=scan_id, card_id=card_id)
            self.service.create_deck_entry(
                {
                    "cardID": card_id,
                    "sourceScanID": scan_id,
                    ...
                    "selectedRank": 1,
                    "wasTopPrediction": True,
                    "addedAt": f"2026-04-14T20:2{kind_index}:00Z",
                    "userOverrodeMismatchWarning": kind,
                }
            )
            row = self.service.connection.execute(
                "SELECT user_overrode_mismatch_warning FROM scan_events WHERE scan_id = ? LIMIT 1",
                (scan_id,),
            ).fetchone()
            assert row is not None
            self.assertEqual(row["user_overrode_mismatch_warning"], kind)

        # Omitted field → NULL (the common, no-warning-active path).
        baseline_scan_id = "scan-mismatch-override-baseline"
        baseline_card_id = "mismatch-card-baseline"
        self._seed_scan_event_for_artifacts_json(
            scan_id=baseline_scan_id, card_id=baseline_card_id
        )
        self.service.create_deck_entry(
            {
                "cardID": baseline_card_id,
                "sourceScanID": baseline_scan_id,
                "selectionSource": "top",
                "selectedRank": 1,
                "wasTopPrediction": True,
                "addedAt": "2026-04-14T20:25:00Z",
            }
        )
        baseline_row = self.service.connection.execute(
            "SELECT user_overrode_mismatch_warning FROM scan_events WHERE scan_id = ? LIMIT 1",
            (baseline_scan_id,),
        ).fetchone()
        assert baseline_row is not None
        self.assertIsNone(baseline_row["user_overrode_mismatch_warning"])

    def test_create_deck_entry_rejects_invalid_user_overrode_mismatch_warning(self) -> None:
        scan_id = "scan-mismatch-override-invalid"
        card_id = "mismatch-card-invalid"
        self._seed_scan_event_for_artifacts_json(scan_id=scan_id, card_id=card_id)

        with self.assertRaisesRegex(ValueError, "userOverrodeMismatchWarning"):
            self.service.create_deck_entry(
                {
                    "cardID": card_id,
                    "sourceScanID": scan_id,
                    ...
                    "selectedRank": 1,
                    "wasTopPrediction": True,
                    "addedAt": "2026-04-14T20:30:00Z",
                    "userOverrodeMismatchWarning": "bogus",
                }
            )
