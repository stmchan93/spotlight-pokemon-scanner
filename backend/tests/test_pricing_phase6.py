...
class TopMoversTests(unittest.TestCase):
    def setUp(self) -> None:
        self.tempdir = tempfile.TemporaryDirectory()
        self.database_path = Path(self.tempdir.name) / "top_movers.sqlite"
        self.connection = connect(self.database_path)
        apply_schema(self.connection, BACKEND_ROOT / "schema.sql")
    ...
    def _seed_card_with_history(
        ...
        *,
        ...
        prior_date = (datetime.now(timezone.utc).date() - timedelta(days=days_ago + 1)).isoformat()
        current_date = (datetime.now(timezone.utc).date() - timedelta(days=days_ago)).isoformat()
        for price_date, market_price in ((prior_date, prior_price), (current_date, current_price)):
            upsert_price_history_daily(
                self.connection,
                card_id=card_id,
                pricing_mode=RAW_PRICING_MODE,
                ...
                variant="Holofoil",
                condition="NM",
                low_price=market_price - 0.5,
                market_price=market_price,
                mid_price=market_price,
                high_price=market_price + 0.5,
                payload={"provider": "scrydex"},
            )

    def test_top_movers_returns_gainers_sorted_by_percent_change(self) -> None:
        self._seed_card_with_history(card_id="set-a", name="Big Gainer", prior_price=10.0, current_price=15.0)  # +50%
        self._seed_card_with_history(card_id="set-b", name="Small Gainer", prior_price=20.0, current_price=22.0)  # +10%
        self._seed_card_with_history(card_id="set-c", name="Mid Gainer", prior_price=8.0, current_price=10.0)  # +25%
        self.connection.commit()

        service = SpotlightScanService(self.database_path, REPO_ROOT)
        try:
            payload = service.top_movers(limit=10)
        finally:
            service.connection.close()

        names = [m["name"] for m in payload["movers"]]
        self.assertEqual(names, ["Big Gainer", "Mid Gainer", "Small Gainer"])

        first = payload["movers"][0]
        self.assertEqual(first["cardID"], "set-a")
        self.assertAlmostEqual(first["currentPrice"], 15.0)
        self.assertAlmostEqual(first["priorPrice"], 10.0)
        self.assertAlmostEqual(first["changeAmount"], 5.0)
        self.assertAlmostEqual(first["changePercent"], 50.0)
        self.assertEqual(first["currencyCode"], "USD")
        self.assertIsNotNone(first["currentDate"])
        self.assertIsNotNone(first["priorDate"])

    def test_top_movers_excludes_cards_below_min_prior_price(self) -> None:
        self._seed_card_with_history(card_id="cheap", name="Penny Card", prior_price=0.10, current_price=1.00)  # +900% but excluded
        self._seed_card_with_history(card_id="qualifies", name="Eligible Card", prior_price=10.0, current_price=11.0)
        self.connection.commit()

        service = SpotlightScanService(self.database_path, REPO_ROOT)
        try:
            payload = service.top_movers(limit=10, min_prior_price=5.0)
        finally:
            service.connection.close()

        names = [m["name"] for m in payload["movers"]]
        self.assertEqual(names, ["Eligible Card"])

    def test_top_movers_excludes_losers_and_flat_cards(self) -> None:
        self._seed_card_with_history(card_id="winner", name="Winner", prior_price=10.0, current_price=12.0)
        self._seed_card_with_history(card_id="loser", name="Loser", prior_price=10.0, current_price=8.0)
        self._seed_card_with_history(card_id="flat", name="Flat", prior_price=10.0, current_price=10.0)
        self.connection.commit()
    ...
    def test_top_movers_excludes_stale_cards_beyond_max_age(self) -> None:
        self._seed_card_with_history(card_id="recent", name="Recent", prior_price=10.0, current_price=12.0, days_ago=0)
        self._seed_card_with_history(card_id="stale", name="Stale", prior_price=10.0, current_price=20.0, days_ago=30)
        self.connection.commit()
    ...
    def test_top_movers_limit_clamps_result_count(self) -> None:
        for index in range(5):
            self._seed_card_with_history(
                card_id=f"card-{index}",
                name=f"Card {index}",
                prior_price=10.0,
                current_price=10.0 + (index + 1),
            )
        self.connection.commit()

        service = SpotlightScanService(self.database_path, REPO_ROOT)
        try:
            payload = service.top_movers(limit=2)
        finally:
            service.connection.close()

        self.assertEqual(len(payload["movers"]), 2)
