#!/usr/bin/env python3
"""Phase 5 migration: drop the 4 JSON columns from ``card_price_history_daily``
via a staged slim-table swap.

The JSON columns (``raw_contexts_json``, ``graded_contexts_json``, ``source_url``,
...
Phase-5 writer omits them when absent). An in-place ``ALTER TABLE ... DROP COLUMN``
rewrites the whole table while holding a write lock and is outage-prone on a
multi-GB table, so instead this does a controlled rebuild:

  1. CREATE a ``card_price_history_daily_slim`` table with every column EXCEPT the
     4 JSON ones (same PK, same column types).
