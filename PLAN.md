...
- The current locked React Native parallel execution source of truth is [docs/react-native-parallel-execution-spec-2026-04-21.md](/Users/stephenchan/Code/spotlight/docs/react-native-parallel-execution-spec-2026-04-21.md).
...
### Milestone: Scanner + visual-model rewrite (RN-era long-term loop)

Status: `active`

- Source of truth: [docs/scanner-model-rewrite-spec-2026-04-23.md](/Users/stephenchan/Code/spotlight/docs/scanner-model-rewrite-spec-2026-04-23.md)
- Paired data loop: [docs/scan-data-labeling-pipeline-spec-2026-04-23.md](/Users/stephenchan/Code/spotlight/docs/scan-data-labeling-pipeline-spec-2026-04-23.md)
- Goal:
  - fix scan-to-scan inconsistency in the front half
  - grow training corpus via a production scan → friend-label → training-example loop
  - raise top-1 without OCR regressions
  - keep iOS shipping unchanged, bridge cleanly to the RN parallel rewrite
  - make OCR eventually removable from raw once visual retrieval clears the removal gates
- Phase order:
...
- Hard rules:
  - no artwork-only crops
  - no training on `qa/raw-footer-layout-check/`
  - no broadening runtime `top-K` above 10 this cycle
  - no rewrite of the scanner in pure JS for RN; camera/vision stays native
