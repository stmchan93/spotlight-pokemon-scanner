...
5b. **Pricing cutover (added 2026-09-08).** The deploy flips prod's headline raw price to TCGplayer (`TCGCSV_SYNC_ENABLED=1`, `RAW_MAIN_PRICE_SOURCE=tcgcsv` in `.env.production`) and ships the JP TCGplayer-id backfill map. The Top Trends feed block and the per-day price graphs only compare like with like, so prod ALSO needs 30+ days of TCGplayer history in the main lane or they fall back to Scrydex pairs for a month and the trend's "now" ≠ the card's price. If step 2 copies staging's DB, that history rides along (staging replayed 2026-08-07..09-07 from TCGCSV's daily archives on 09-08). If not, run `backend/backfill_tcgcsv_history.py` on the prod VM after the deploy — exact commands, timing (~3-5 h) and verification are in the checklist (Phase 2, "Pricing cutover"). Never run it BEFORE the deploy: the main-lane columns don't exist on prod's DB until then.
