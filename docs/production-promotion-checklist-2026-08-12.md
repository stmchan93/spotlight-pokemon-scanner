...
- [ ] **[BLOCKER] Pricing cutover to TCGplayer (added 2026-09-08).** The deploy carries `TCGCSV_SYNC_ENABLED=1` + `RAW_MAIN_PRICE_SOURCE=tcgcsv` in `backend/.env.production` (staged 09-07 after a 2-week staging soak) and `backend/tcgplayer_id_backfill.json` (4,075 JP vintage/promo → TCGplayer ids; Scrydex never linked them). After deploy the 13:05 PT cron prices ~45k cards/day from TCGplayer. **Do not skip the history step below or Top Trends and the price graphs compare Scrydex pairs for a month while the card shows a TCGplayer headline.**
  - [ ] **If Phase 2 copies staging's SQLite to prod:** the 32-day TCGplayer history (2026-08-07..09-07, replayed on staging 09-08) rides along in the copy — nothing to run. Verify with `GET /api/v1/market/top-movers` → 5 games × 5 items, `source: main_raw`.
  - [ ] **If the deploy is code-only on prod's own DB:** run the TCGCSV history backfill on the prod VM (needs explicit go + `SPOTLIGHT_PROD_CONFIRM=yes`): (1) if the deploy lands after ~2026-09-20, re-extract so the files cover today−32..yesterday: `python3 tools/extract_tcgcsv_archives.py --start <today-32> --end <yesterday> --out-dir <dir>` (laptop, needs `brew install sevenzip`; reuse `--skip-products` if products.json.gz is fresh); (2) `gcloud compute scp <dir>/*.gz spotlight-backend-vm-small:/home/stephenchan/tcgcsv-history/ --zone us-central1-c`; (3) on the VM, source `.vm-runtime.conf` + env file, `nohup ./.venv/bin/python backfill_tcgcsv_history.py --database-path $SPOTLIGHT_DATABASE_PATH --prices-dir /home/stephenchan/tcgcsv-history > /home/stephenchan/tcgcsv-history/backfill.log &` — **~5-6 min/day ≈ 3-5 h** (index-bound on the cell table), API keeps serving, idempotent if interrupted, ~1-1.5 GB disk (prod had 68 GB free on 09-08); (4) then `run_tcgcsv_sync_vm.sh` once with `--force` (or wait for the 13:05 PT cron) so TODAY's snapshot main lane is populated; (5) verify `/api/v1/market/top-movers` shows 30-day movers with `source: main_raw`, and a JP vintage card (e.g. `gym1_ja-40` Lt. Surge's Magneton) serves `mainPriceSource: tcgcsv` ≈ $14, not $57.
  - [ ] Confirm `MARKET_MOVERS_WINDOW_DAYS` is UNSET on prod (default 30). Staging ran 10 for one day on 09-08; the override is gone.
